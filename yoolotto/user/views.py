    ...
    def post(self, request):
        ...
        if email_source == "app":        
            if hashers.check_password(password, user.password):
                ...
                if created:
                    email_coins.dollar_amount =0.01
                    email_coins.save()
            ...
            if email_source == "twitter":
                ...
                try:
                    email_coins = EmailCoins.objects.get(email = data['fb_email'])
                except:
                    email_coins,created = EmailCoins.objects.get_or_create(email = data['email'],defaults={"coins":6})
                    if created:
                        email_coins.dollar_amount=0.06
                        email_coins.save()
...
class Home(View):        
    ...
    def post(self, request):
        ...
        try:
            device_resp = DeviceCoins.objects.filter(device_id = request.yoo['device'])[0]
            print "device details",device_resp
            print device_resp.coins_transfered
            if device_resp.coins_transfered == 0:
                print "in looop"
                email_resp = EmailCoins.objects.get(email = header_email)
                print email_resp
                email_resp.coins = email_resp.coins + device_resp.coins
                email_resp.save()
                device_resp.coins_transfered =1
                device_resp.save()
        except:
            pass
	...
	yoo_prize_code =  0
        yoo_game_code =  0
        print "coinsssssssssss are",coins
