...
from yoolotto.second_chance.models import FiberCoins,ResetCoins
...
class RegisterUser(View):
    ...
    def post(self,request):
        ...
        print "userrrrrrrrrrrrrrrrr_data",user_data
...
class LogoutUser(View):
    ...
    @Authenticate()
    def post(self, request):
        UserClientLogin.objects.filter(device=request.yoo['device']).update(client_login=None)
...
class User(View):
        ...
        if request.yoo["new_version"]:
            ...
            try:
                ...
                try:
                    ...
                    preferences = user.preferences
    	    try:                
            	ClientLoginRecord = UserClientLogin.objects.filter(device=request.yoo['device'],client_login = data['email'])[0]
    	    except:
            ...
            print "fibeeeeeeeeeeee",fiber_coins
            print "ligin details",ClientLoginRecord.client_login
            if ClientLoginRecord.client_login:
                ...
                try:
                    fiber_coins_record = FiberCoins.objects.filter(email_id = coins_record.id)[0]
                except:
                    fiber_coins_record , created = FiberCoins.objects.get_or_create(email_id = coins_record.id)
                fiber_coins = fiber_coins_record.offer_wall + fiber_coins_record.rewarded_video
                print "fibbbbbbbbbbbbbbbbbbb",fiber_coins
                coins1 = coins_record.coins + device_coins_record.get_coins()
                print "cccccccccccc",coins1
                tickets = LotteryTicketClient.objects.filter(email=ClientLoginRecord.client_login).count()
            else:
                coins1 = device_coins_record.coins
            tickets = LotteryTicketClient.objects.filter(device=request.yoo['device']).count()
            coins = coins1 + fiber_coins
            print "mainnnnnnnnnn coins",coins
            ...
            if request.yoo["new_version"]:
                #coins = wallet.coins
                print "coinnnnnnnnnnnnnnnnnaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",coins
                return {
                "email": user.email,
	...
	if request.yoo["new_version"]:
         ...
         print "emailll in user",email
         print "Password in user",password
         # Routing Logic        
         ...
         if email:
            ...
            print "coinsssssss",coins_record.email
            coins_record.save()
		...
		ClientLoginRecord  = UserClientLogin.objects.filter(device=request.yoo['device'])[0]
         ...
         print "client login111111111111111111111111111",ClientLoginRecord.client_login
         print "client deviceeeeeeeeeeeee",ClientLoginRecord.device
