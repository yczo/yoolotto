...
from yoolotto.coin.manager import CoinTicketManager, CoinTicketManagerNew
...
class LotteryTicket(models.Model):
    ...
    def update_new_release(self, multicoin,count_tick, device=None, Client_login=None, full=False):
        if full:
            self.calculate_winnings()
        if not multicoin:
            allocated = self.update_coins_new_release(device,count_tick, Client_login, save=False)
        else:
            allocated = 0
    ...
    def update_coins_new_release(self, device,count_tick, Client_login, save=False):
        manager = CoinTicketManagerNew(device,count_tick, Client_login, ticket=self)
        manager.update_new_release()
        if save:
            self.save()
    ...
    def representation_old(self,
        representation1=representations.TicketRepresentationShortListOld,
        *args, **kwargs):
        return representation_old(self)(*args, **kwargs)
            
    def representation(self,email,
        representation=representations.TicketRepresentationShortList,
        *args, **kwargs):
        return representation(self,email)(*args, **kwargs)
