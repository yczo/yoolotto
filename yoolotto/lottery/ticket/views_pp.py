	...
	try:
		...
		availability.rejected = True
                ...
                for line in lines:
                    ...
                    available_record = LotteryTicketAvailable(available = True, json= line, ticket_id= valid_ticket, play_id=play_record.id, device = availability.device, image_first = "static/ticket/"+str(play_id)+"_1.jpg", defected = defected_ticket,pending=False)
		...
		play_record, created = LotteryTicketPlay.objects.get_or_create(play=previous_play.play,ticket=multi_ticket_info,division=LotteryCountryDivision.objects.get(remote_id=division))
                play_record.submission_old = submission
                play_record.submission = submission_record
            
                play_record.update_winnings(save=True)

                #lines[0]['gameType'] = game_type

                available_record = LotteryTicketAvailable(available = True, json= '', ticket_id= valid_ticket, play_id=play_record.id, device = availability.device, image_first = "static/ticket/"+str(play_id)+"_1.jpg",pending=False)

                available_record.save()

		'''available_record = LotteryTicketAvailable(available = True, ticket_id = valid_ticket, play_id = play_record.id,  device = availability.device)
		available_record.save()'''
                ...
                for line in lines:
                    ...
                    available_record = LotteryTicketAvailable(available = True, json= line, ticket_id= valid_ticket, play_id=play_record.id, device = availability.device, image_first = "static/ticket/"+str(play_id)+"_1.jpg",pending=False)
