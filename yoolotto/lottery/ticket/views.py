	...
	RetailerNo = ''
        try:
      	...
      	if state == 'TX' and old_version == False:
            ocr_lib = cdll.LoadLibrary("/root/yoolotto/yoolotto/OCR-OCR_server/YoolottoOCR/libOCR.so") # pass the path of .so file
            ...
            lib_prt_path ="/root/yoolotto/yoolotto/OCR-OCR_server"
        ...
        url = "http://172.16.240.6/TestService/DataEntryTool.WcfService.TicketService.svc/AddTicket"
        try:
    ...
    #version = request.META["HTTP_YOO_APP_VERSION"]
	    ...
	    ticket_user = ticket_info.user
            ...
            if version == "6.0": 
                ...
                for ticket in tickets.values():
                        allocated[ticket.pk] = ticket.update_new_release(multicoin,countt,availability.device,user.email)
                        #multicoin = 1
            if version <= "4.2.6":
                result = map(lambda x: x.representation_old(ticket_device,allocated=allocated), tickets.values())
            else:
