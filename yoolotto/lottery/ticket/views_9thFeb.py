...
def send_data_to_det(success,ticket,play,game_type,image_data,state,old_version):
    if success:
        ...
        if state == 'TX' and old_version == False:
            ...
            updated_ocr_data = str({"lines":lines,"drawTime":json_data['drawTime']})
        ...
        data = {"StateId":ticket.division_id,"DrawDate":str(ticket.draw.date),"GameTypeId":valid_data_entry_game_type,"PlayId":play.id,"ImageName":str(play.id)+"_1.jpg","OCRRowData":updated_ocr_data,"TicketId":ticket.id}
	...
	try:
	    ...
	    availability.available = True
            ...
            for each_date in all_dates:
                date = datetime.datetime.strptime(each_date, "%Y-%m-%d").date()
                updated_draw, created = LotteryDraw.objects.get_or_create(component=component,division_id=common_state,date=date)
                multi_draws.append(updated_draw)

                multi_ticket_info, created = LotteryTicket.objects.get_or_create(deleted=False,user=ticket_info.user,draw=updated_draw,division=LotteryCountryDivision.objects.get(remote_id=division))
                valid_draw = updated_draw.id
    ...
    def post(self, request):
        ...
        if game_type == 3 or game_type == 4:
            ...
            for record in data['lines']:
                ...
                game = games[game_type]#LotteryGame Obj
                handler = game.handler 
                '''record['lines'][0]['numbers'] = numbers
                record = handler.preprocess_ticket(record)
                lines = handler.validate_play(record['lines'][0])'''
...
class Ticket(View):
	...
	state = division
        ...
        if not _filter:
            ...
            _filter = True
	#unavailable_tickets = LotteryTicketAvailable.objects.filter(available=False).values_list('ticket',flat=True)        

	unavailable_tickets = []
        det_tickets = LotteryTicketAvailable.objects.filter(ticket__user_id=request.yoo['user'])
        
	for item in det_tickets:
            
            if item.available == False:
                unavailable_tickets.append(item.ticket_id)
            else:
		try:
		     unavailable_tickets.remove(item.ticket_id)
		except:
		     pass
	    ...
	    tickets = list(user.tickets.filter(division_id=division.id,deleted=False).exclude(winnings=None).order_by("-draw__date") | user.tickets.filter(division_id=division.id, draw__result=None).order_by("-draw__date"))
	    ...
	    if state == 'CA':
            ...
            tickets = user.tickets.filter(draw__component__parent=game,division_id=division.id,deleted=False).exclude(winnings=None).order_by("-draw__date") | user.tickets.filter(draw__component__parent=game,division_id=division.id, draw__result = None).order_by("-draw__date")
