...
def send_data_to_det(success,ticket,play,game_type,image_data,state,old_version,data):
    if success:
        ...
        if state == 'TX' and old_version == False:
            ...
            EndDate = json_data['drawdateTo'] or data[0]["multiDrawEndDate"]
            if data[0]['gameType'] == 3 or data[0]['gameType'] == 4:
                if Consecutive == 1:
                    #startdrawTime = json_data['drawTimeFrom'] or data[0]['startdrawTime']
                    enddrawTime = json_data['drawTimeTo'] or data[0]['enddrawTime']
                else:
                    pass
        ...
        try:
            if multiDraw and Consecutive == 1:
                data = {"StateId":ticket.division_id,"DrawDate":str(ticket.draw.date),"GameTypeId":valid_data_entry_game_type,"PlayId":play.id,"ImageName":str(play.id)+"_1.jpg","OCRRowData":updated_ocr_data,"TicketId":ticket.id,"StartDate":StartDate,"EndDate":EndDate,"MultiCheck":multiDraw,"Consecutive":1}
            elif multiDraw:
                data = {"StateId":ticket.division_id,"DrawDate":str(ticket.draw.date),"GameTypeId":valid_data_entry_game_type,"PlayId":play.id,"ImageName":str(play.id)+"_1.jpg","OCRRowData":updated_ocr_data,"TicketId":ticket.id,"StartDate":StartDate,"EndDate":EndDate,"MultiCheck":multiDraw,"Consecutive":0}
            ...
            data = {"StateId":ticket.division_id,"DrawDate":str(ticket.draw.date),"GameTypeId":valid_data_entry_game_type,"PlayId":play.id,"ImageName":str(play.id)+"_1.jpg","OCRRowData":'',"TicketId":ticket.id}
