	...
	RetailerNo = ''
        try:
      	...
      	if state == 'TX' and old_version == False:
            ocr_lib = cdll.LoadLibrary("/home/kanika/yoolotto_multi/yoolotto/OCR-OCR_server/YoolottoOCR/libOCR.so") # pass the path of .so file
            ...
            lib_prt_path ="/home/kanika/yoolotto_multi/yoolotto/OCR-OCR_server"
...
class SubmitAgain(View):
	...
	#version = request.META["HTTP_YOO_APP_VERSION"]
		...
		availability.rejected = True
                ...
                # create new ticket
                game_type_list = [4,3,7]
                if game_type in game_type_list and data["Consecutive"] == 0:
		...
		pass
            ...
            dev_id = Device.objects.get(device_id = device_id)
            version = dev_id.app_version
            if version <= "4.2.6":
                result = map(lambda x: x.representation1(allocated=allocated), tickets.values())
            else:
                result = map(lambda x: x.representation(allocated=allocated), tickets.values())
            for _result in result:
                    coins = allocated.get(_result["ticket_id"], None)
...
class Ticket(View):
	...
	state = division
        ...
        #fantasy_tickets = list(user.fantasy_tickets_user.filter(division = division,deleted=False).order_by("-draw__date") | user.fantasy_tickets_user.filter(division = division,deleted=False).order_by("-draw__date").exclude(draw__result=None))
        elif draw_id:
...
class TicketCheck(View):
    ...
    def post(self, request, _id):
        ...
        print "result in check ticket"._result
...
class enumeration_data(View):
    ...
    @Authenticate()
    def get(self,request):
        NUMBERS_MONEY={"Powerball":["5 numbers +ball $1,000,000 cash","5 numbers       $    5,000 cash","4 numbers +ball $      500 cash","4 numbers       $       10 cash","3 numbers +ball @   50 yoocoins","3 numbers -     @    5 yoocoins","2 numbers +ball @    5 yoocoins","1 numbers +ball @    2 yoocoins","                @    1 yoocoins"],"Megamillion":["5 numbers +ball $1,000,000 cash","5 numbers       $    5,000 cash","4 numbers +ball $      500 cash","4 numbers       $       10 cash","3 numbers +ball @   50 yoocoins","3 numbers -     @    5 yoocoins","2 numbers +ball @    5 yoocoins","1 numbers +ball @    2 yoocoins","                @    1 yoocoins"]}
        user = request.yoo["user"]
        ...
        try:
            fiber_coins_record = FiberCoins.objects.filter(device_id = device_coins_record.id)[0]
        except:
            fiber_coins_record , created = FiberCoins.objects.get_or_create(device_id = device_coins_record.id)
        fiber_coins = fiber_coins_record.offer_wall + fiber_coins_record.rewarded_video
        if ClientLoginRecord.client_login:
            ...
            fiber_coins_record , created = FiberCoins.objects.get_or_create(email_id = coins_record.id)
            fiber_coins = fiber_coins_record.offer_wall + fiber_coins_record.rewarded_video
        try:
            coins = coins_record.coins + device_coins_record.coins
        except:
            coins = device_coins_record.get_coins()
        total_coins = coins + fiber_coins  
...
class SubmitNumbers(View):
    ...
    def post(self,request):
        ...
        for line in numbers:
            ...
            play_record.update_winnings(save=True)
            play_record.save()
            ticket_info.winnings = F('winnings') + float(play_record.winnings or 0)
            ticket_info.winnings_coins = F('winnings_coins') + float(play_record.winnings_coins or 0)
            ticket_info.save()
        ClientLoginRecord = UserClientLogin.objects.filter(device=request.yoo['device'])[0]
        game_coins = coins
        if ClientLoginRecord.client_login:
            coins_info = EmailCoins.objects.filter(email = ClientLoginRecord.client_login)[0]
            if coins_info.coins > game_coins:
                coins_info.coins = F('coins')-game_coins
        ...
        return {"success":True,"coins":game_coins}
