...
class SubmitAgain(View):
	...
	end_component = 1
		...
		serialized = dumps(lines[0])
                play_record = LotteryTicketPlay(play=serialized,ticket=multi_ticket_info,division=LotteryCountryDivision.objects.get(remot$
                play_record.submission_old = submission
                play_record.submission = submission_record

                play_record.update_winnings(save=True)

                available_record = LotteryTicketAvailable(available = True, json= lines[0], ticket_id= valid_ticket, play_id=play_record.i$
	    ...
	    if data["multiCheck"]:
		    ...
		    available_record = LotteryTicketAvailable(available = True, json= lines[0], ticket_id= valid_ticket, play_id=play_record.id, device = availability.device, image_first = "static/ticket/"+str(play_id)+"_1.jpg",pending=False)
