...
class SpecificTicketCheck(View):
	...
	#	ticket = draw.tickets.filter(user=user,division__remote_id=division)[0]
        ...
        if ticket.draw.result:
            ...
            allocated = ticket.update_new_release(request.yoo["device"],request.META['HTTP_YOO_EMAIL_ID'],full=True)
            
        ticket = LotteryTicket.objects.get(pk=ticket.pk)
        if data["ticketcheck_coins_deduction"] == 1 and ticket.fantasy != 1:
            email = request.META['HTTP_YOO_EMAIL_ID']
            device = request.yoo["device"]
            deduct_coins_on_ticketcheck(email)
            ticket.coins_deduction = 1
            ticket.save()
        ...
        if ticket.add_coins == 0 and ticket.fantasy != 0:        
            ...
            amount = round((float(ticket.winnings_coins)/100),2)
        ...
        if ticket.add_winnings == 0 and ticket.fantasy != 0:
            ...
            coins_record.dollar_amount = round((coins_record.dollar_amount + ticket.winnings),2)
