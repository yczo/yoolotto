...
class SCA_url(View):
    ...
    @Authenticate()
    def post(self, request):
        raise exceptions.WebServiceException("Service Unavailable")
...
'''class SCA_url(View):
    ...
    def post(self, request):
        ...
        if request.META["HTTP_YOO_APP_VERSION"] <=5.1:
        	try:
        	    client_login_record, created = UserClientLogin.objects.get_or_create(device=request.yoo['device'],client_login = request.META["HTTP_YOO_EMAIL_ID"])
        	except:
        	    client_login_record = UserClientLogin.objects.filter(device=request.yoo['device'],client_login = request.META["HTTP_YOO_EMAIL_ID"])[0]
        	try:
        	    device_coins,created = DeviceCoins.objects.get_or_create(device_id=client_login_record.device)
        	except:
        	    device_coins = DeviceCoins.objects.filter(device_id=client_login_record.device)[0]
        	if client_login_record.client_login == None:
        	    coins = device_coins.coins
        	else:
        	    email_record, created = EmailCoins.objects.get_or_create(email=request.META["HTTP_YOO_EMAIL_ID"], defaults={'coins': 0})
                coins = device_coins.coins + email_record.coins
        else:
            try:
...
class InviteFriends(View):
    ...
    def post(self, request):
        ...
        if data['invites']:
            coins_record.coins = coins_record.coins + float(data['invites'])*1
            coins_record.invites = int(coins_record.invites) + int(data['invites'])
            amount = float((data['invites'])*1)/100
            coins_record.dollar_amount = coins_record.dollar_amount + amount
            coins_record.save()
            invite_coins = float(data['invites'])*1
    ...
    def post(self,request):
	    ...
	    dwolla_info.isDwollaUser = 1
            ...
            if data['source'] == "coins":
                ...
                print "coins before",coins_info.coins
                ...
                print "coins after",coins_info.coins
                coins_info.save()
