...
//    printf("uj->pos[0]= %d, uj->ncomp= %d\n",uj->pos[0],uj->ncomp);

    if (uj->pos[0] != uj->ncomp) ujThrow(UJ_UNSUPPORTED);
    ujSkip(uj, 1);
//   printf("comp, uj->ncomp= %d\n",uj->ncomp);
...
void reloadRGBBitmap(LPBYTE pixPtr, int wPadOut, LPBYTE rgb, int wInp, int height){
	int x,y, h1 = height-1, w3 = wInp*3;
	int numInt, r;
	LPBYTE pInp, pOut;
	int *pIntInp, *pIntOut;
	  numInt =  w3/sizeof(int);
	  r =  w3%sizeof(int);
	  for(y=0;y<height;y++){
		  pIntInp = (int *)(rgb + (h1-y)*w3);
		  pIntOut = (int *)(pixPtr+ y*wPadOut);
		  for(x=0;x<numInt;x++){
			  pIntOut[x] = pIntInp[x];
		  }
		  if(r>0){
			  pInp = (LPBYTE)&pIntInp[numInt];
			  pOut = (LPBYTE)&pIntOut[numInt];
			  for(x=0;x<r;x++){
				  pOut[x] = pInp[x];
			  }
		  }
...
int reloadRGBtoBMP(LPBYTE rgb, int width, int height, LPIMGDATA img){
	int retValue = 0;
	LPBYTE ptr ;
	...
	biWidthOut = width;
	biWidthPadOut = ((width*24+31)/32)*4;
	biHeightOut = height;
	dpi = 500;
	...
	biSizeImage = biWidthPadOut*height + offBits;

// printf("-----BMP biSizeImage= %d \n", biSizeImage);
// fflush(stdout);

	ptr = calloc( biSizeImage, sizeof(BYTE));
	if(ptr == NULL) {
	...
	img->ptr = ptr;
	img->pixPtr = ptr + offBits;
	img->width = biWidthOut;
	img->height = biHeightOut;
	img->bf.bfType = 0x4d42;
	img->bf.bfSize = biSizeImage;
	img->bf.bfReserved1 =  0;
	img->bf.bfReserved2 =  0;
	img->bf.bfOffBits =   offBits;

	img->bi.biBitCount = 24;
	img->bi.biWidthPad = biWidthPadOut;
	img->bi.biHeight = biHeightOut;
	img->bi.biWidth = biWidthOut;
	img->bi.biSize = sizeof (BITMAPINFOHEADER);
	img->bi.biPlanes = 1;

	img->bi.biCompression = BI_RGB;
	img->bi.biSizeImage = biSizeImage - offBits;
    ...
    reloadRGBBitmap(img->pixPtr, biWidthPadOut, rgb, biWidthInp, height);
...
int createBMPfromJPEG(LPBYTE ptrJPG, int length,  LPIMGDATA img){
	...
	int width, hight;
//	ujContext *ujc;
	uj = ujDecode(NULL, ptrJPG, length);
//    ujc = (ujContext*) uj;
    if(uj == NULL || ujGetError() != UJ_OK){
    	printf("createBMPfromJPEG: ERROR: %d",ujError);return -2;
    }
    dest = ujGetImage(uj, NULL);
    if(dest == NULL || ujGetError() != UJ_OK){
    	printf("ujGetImage: ERROR: %d",ujError);return -2;
    }
    width = ujGetWidth(uj);
    hight = ujGetHeight(uj);
//	printf("width= %d, hight= %d\n",width, hight);
	retValue = reloadRGBtoBMP(dest, width, hight, img);
