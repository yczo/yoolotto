...
static BYTE *fp_jpeg_buff;
static int len_JPG_buff;
...
void writebyte(BYTE b) {
	fp_jpeg_buff[len_JPG_buff] = b;
//	if(countWByte < 50)
//	  printf("countWByte= %d, fp_jpeg_buff[len_JPG_buff]= %0x\n", countWByte, fp_jpeg_buff[len_JPG_buff]);
...
//	WORD d = w/256;
	//if(countWByte < 50)
//		printf("w= %0X, d= %0x\n", w, (BYTE)d);
	writebyte((w)/256); writebyte((SBYTE)(w)%256);}
...
void copyBMPLine(BYTE *RGB_buffer, LPIMGDATA img, int line){
	int wPad = ((img->bi.biWidth*img->bi.biBitCount+31)/32)*4;
	LPBYTE ptr = img->pixPtr +(img->bi.biHeight-1-line)*wPad;
	int i;
	for(i=0;i<wPad;i++)
		RGB_buffer[i] = ptr[i];

}
void reload_bitmap(LPIMGDATA img, WORD *Ximage_original, WORD *Yimage_original)
{
 WORD Xdiv8,Ydiv8;
 BYTE nr_fillingbytes;//The number of the filling bytes in the BMP file
     // (the dimension in bytes of a BMP line on the disk is divisible by 4)
...
// if ((TMPBUF[0]!='B')||(TMPBUF[1]!='M')||(TMPBUF[28]!=24))
//	 exitmessage("Need a truecolor BMP to encode.");
 Ximage=img->bi.biWidth;Yimage=img->bi.biHeight;
 *Ximage_original=Ximage;*Yimage_original=Yimage; //Keep the old dimensions
						// of the image
 if (Ximage%8!=0) Xdiv8=(Ximage/8)*8+8;
	else Xdiv8=Ximage;
...
//	 copyBMPLine((BYTE *)RGB_buffer+nrline*Xdiv8, img, nrline);
	 copyBMPLine((BYTE *)(RGB_buffer+nrline*Xdiv8), img, nrline);
  }
 Ximage=Xdiv8;
 dimline=Ximage*3;
...
// memcpy(tmpline,RGB_buffer+(Yimage-1)*Ximage,dimline);
 for (nrline=Yimage;nrline<Ydiv8;nrline++)
  {memcpy(RGB_buffer+nrline*Ximage,RGB_buffer+(Yimage-1)*Ximage,dimline);}
 Yimage=Ydiv8;
// free(tmpline);fclose(fp_bitmap);
}
...
//	load_bitmap(BMP_filename, &Ximage_original, &Yimage_original);
	 reload_bitmap(img,  &Ximage_original, &Yimage_original);
...
//	fp_jpeg_stream=fopen(JPG_filename,"wb");
	 fp_jpeg_buff=(BYTE *)malloc(500000*sizeof(BYTE));
	 if (fp_jpeg_buff==NULL){ printf("Not enough memory.\n"); return 0;}
	...
	SOF0info.height=Yimage_original;
	writeword(0xFFD8);//SOI
//	printf("1 = %0x, 2= %0x\n", 0xFFD8/256, 0xFFD8%256);


//printf("lenJPGbuffstream= %d\n", len_JPG_buff);
	write_APP0info();
//	printf("lenJPGbuffstream=  %d\n", len_JPG_buff);
//	printf("len_JPG_buff= %d\n", len_JPG_buff);
	write_DQTinfo();
//	printf("len_JPG_buff= %d\n", len_JPG_buff);
...
void setNewImageNameJPG2(char *imgName, char *imgNInp){
...
void saveJPGbuffer (LPBYTE buff, int len, char* name, char* path)
{
	...
	f= fopen(fileName, "w");
	if(f == NULL) {
		fprintf(stderr,"saveJPGbuffer (\"%s\") failed: %s\n",fileName, strerror(errno));
		return;
	}
	...
	if(ferror(f) || fSize*len != len){
		if(ferror(f)) {
			fprintf(stderr,"saveJPGbuffer (\"%s\") fwrite failed: %s\n",fileName, strerror(errno));
		}
		else {
			fprintf(stderr,"saveJPGbuffer (\"%s\") fwrite failed: (size= %d) != (len= %d\n)",fileName, fSize*len, len);
		}
		return;
	}
	close_file(f);
