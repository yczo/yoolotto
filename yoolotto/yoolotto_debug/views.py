...
def send_bulk_notification(req):
    if req.POST.get("send_notification",False):
      ...
      if device_type=='all':
        ...
        tokens=Device.objects.filter(device_token__isnull=False,device_type=device_type.upper()).exclude(device_token='').order_by('-id').values_list('device_token',flat=True).distinct()
  
      if not tokens:
        log="tokens not found"
        return log
      log=log+"total tokens found=(%s)"%tokens.count()
      for token in tokens:
        ...
        if device.is_ios():
          ...
          gcm.send()
      return render_to_response("yoolotto_debug/notification.html",data)
    return render_to_response("yoolotto_debug/notification.html")